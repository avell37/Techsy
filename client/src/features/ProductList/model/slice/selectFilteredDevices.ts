...
const selectSelectedBrand = (state: RootState) =>
    state.brandReducer.selectedBrand;
...
export const selectFilteredDevices = createSelector(
    ...
    (devices = [], selectedBrand, selectedType, sortType, search) => {
        ...
        let filtered = devices.filter((device) => {
            const matchedSearch = device.name
                ?.toLowerCase()
            ...
            const matchedBrand =
                selectedBrand.name !== "Бренд"
                    ...
                    : true;
            const matchedType =
                selectedType.name !== "Тип"
                    ? device.typeId === selectedType.id
                    : true;
